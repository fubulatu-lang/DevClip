...
Three alternatives were investigated in depth and rejected: background clipboard reads (blocked at the framework level, and the widely-repeated `appops` workaround is a no-op), shipping an IME (the default-keyboard exemption is real and package-wide, but there is one slot and taking it means replacing Gboard), and Shizuku (works, but dies on every reboot for non-root users and needs a second app installed). The reasoning is preserved in `DECISIONS.md`.
